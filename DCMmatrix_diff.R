...
PPTHRESHOLD <- 0.73
df.CMPM <- df.CMPM %>% mutate(Ep2 = ifelse(Pp<0.73, 0, Ep))
...
#write for illustrations figure 1 (network_node_connections_illustration R file)
write.csv(DCM.matrix2, file = "~/Desktop/edge_info_diff.csv")

#### Create the plot
ggplot(data = melt(DCM.matrix, na.rm = TRUE), aes(Var1, Var2, fill = value)) +
  geom_tile(color = "gray") +
  scale_fill_gradient2(low = "purple4", high = "darkorange2", mid = "white", 
